...
注意:**API Key 只下发给官方域(civitai.com / civitai.green)**;默认站点 civitai.red 等镜像不认 civitai.com 的 Key,带了 Bearer 头会被拒成 403(实测),因此自动不下发。下载遇到 401/403 时,插件会自动依次尝试带 token 与官方域换源。
